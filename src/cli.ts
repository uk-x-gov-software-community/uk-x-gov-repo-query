...
import { type LlmProviderName, extractFilterWithLlm, isEmptyFilter } from "./nlp/llm.js";
...
if (isEmptyFilter(filter) && llmProvider !== undefined) {
  process.stderr.write(`Rule parser found no criteria — falling back to ${llmProvider} LLM…\n`);
  try {
    filter = await extractFilterWithLlm(query, llmProvider);
  } catch (err) {
    console.error(`LLM fallback failed: ${err instanceof Error ? err.message : String(err)}`);
    process.exit(1);
  }
}
